...
What I was particulary interested in was whether I could use it in order to implement some sort
...
``` csharp A ViewModel with valiation annotations
...
-   [The Notification Pattern](http://martinfowler.com/eaaDev/Notification.html) by Martin
    Fowler
-   [Domain centric validation with the Notification Pattern](http://codebetter.com/blogs/jeremy.miller/archive/2007/06/13/build-your-own-cab-part-9-domain-centric-validation-with-the-notification-pattern.aspx) by Jeremy D. Miller
...
it contains an implicit conversion to string. 

``` csharp Our notification abstraction
...
Error is a simple class derived from Notification, which
contains no additional code. 

``` csharp Representing an error as a class
...
I've seen some people implementing the Notification
...
using classes. Decide for yourself: 

``` csharp Class based vs enumeration based implementation
bool isError = notification is Error;
//versus
bool isError = notification.Severity == Severity.Error;
```

It's very rare that you only have to validate one element / property. Mostly we're
...
you a nice place for some additional functionality. 

``` csharp A container for Notifications
...
While the first properties are pretty obvious, the last method probably needs some
...
A Validator<T> using DataAnnotations
---------------------------------------
``` csharp A dedicated Validator
...
``` csharp The PropertyValidator class
...
``` csharp Getting the name that is displayed for a property
...
``` csharp Extracting all DataAnnotation validators
...
Bottom line
-------------
It's pretty easy to combine the existing functionality provided by DataAnnotations with the
